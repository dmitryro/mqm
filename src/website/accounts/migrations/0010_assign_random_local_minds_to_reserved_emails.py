...
class Migration(DataMigration):
    ...
    depends_on = (
        ('local_minds', '0008_add_initial_local_minds'),
    )
    ...
    def forwards(self, orm):
        ...
        ReservedEmail = orm['accounts.ReservedEmail']
        LocalMind = orm['local_minds.LocalMind']
        for reserved_email in ReservedEmail.objects.all():
            if reserved_email.local_mind is None:
                local_minds = LocalMind.objects.all()
                local_minds = local_minds.order_by('?')
                already_set_local_minds = [
                    pk for pk in ReservedEmail.objects.all().values_list('local_mind__pk', flat=True)
                    if pk]
                local_minds = local_minds.exclude(pk__in=already_set_local_minds)
                reserved_email.local_mind = local_minds[0]
    ...
    models = {
        ...
        u'accounts.reservedemail': {
            ...
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'local_mind': ('django.db.models.fields.related.OneToOneField', [], {'to': u"orm['local_minds.LocalMind']", 'unique': 'True', 'null': 'True', 'blank': 'True'})
        },
        u'accounts.user': {
