...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        PositiveNews = orm['news.PositiveNews']
        autoslug = PositiveNews._meta.get_field('slug')

        for news in PositiveNews.objects.all():
            slug = autoslug.create_slug(news, add=True)
            news.slug = slug
            news.save()

        ExternalNews = orm['news.ExternalNews']
        autoslug = ExternalNews._meta.get_field('slug')

        for news in ExternalNews.objects.all():
            slug = autoslug.create_slug(news, add=True)
            news.slug = slug
            news.save()
